...
class ZincWorkerImpl(
  ...
  private def compileInternal(
    ...
    def mkNewReporter(mapper: (xsbti.Position => xsbti.Position) | Null) = reporter match {
      case None =>
        new ManagedLoggedReporter(10, logger) with RecordingReporter with TransformingReporter(mapper) {}
      case Some(forwarder) =>
    ...
    val newReporter = mkNewReporter(
      PositionMapper.create(sources.iterator.zip(virtualSources.iterator))
    )
...
object ZincWorkerImpl {
  ...
  private object PositionMapper {
    ...
    def create(sources: Iterator[(os.Path, VirtualFile)]): (xsbti.Position => xsbti.Position) | Null = {
      val buildSources0 = {
        def isBuild(p: os.Path) =
          mill.main.client.CodeGenConstants.buildFileExtensions.contains(p.ext)

        sources.collect({ case (p, vf) if isBuild(p) =>
          ...
          val adjustedFile = lines
            ...
            .getOrElse(sys.error(p.toString))
  ...
  private trait TransformingReporter(
                              optPositionMapper: (xsbti.Position => xsbti.Position) | Null) extends xsbti.Reporter {
  ...
  private object TransformingReporter {
    ...
    private def transformProblem(problem0: xsbti.Problem, mapper: xsbti.Position => xsbti.Position): xsbti.Problem = {
